...
void Fonts_Get5x7(char character, uint8_t columns[5])
{
  unsigned int index;
  ...
  for (index = 0U; index < (sizeof(s_glyphs) / sizeof(s_glyphs[0])); index++)
  {
    if (s_glyphs[index].character == character)
    {
      (void)memcpy(columns, s_glyphs[index].columns, 5U);
      return;
    }
  }
  (void)memset(columns, 0, 5U);
}
